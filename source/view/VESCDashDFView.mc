...
class DFView extends WatchUi.View {
  ...
  function initialize() {
    ...
    if (
      ...
      vescData.displayNorth == true ||
      vescData.displayWind == true
    ) {
      reqPos = true;
      enableGPS();
    }
  ...
  function getfieldValues() {
    if (reqPos == true && vescData.GPS_requested == false) {
      enableGPS();
    }
    if (reqPos == true) {
    ...
    if (reqWX == true && hasWX == true) {
      if (weatherReqTimer == null || weatherReqTimer < 0) {
        ...
        weatherReqTimer = weatherReqTimer - vescData.updateDelay;
      }
    }
    ...
    for (var field_id = 0; field_id < fieldNB; field_id++) {
      if (fieldIDs[field_id] == 0) {
        ...
        fieldValues[field_id] = valueRound(vescData.correctedSpeed, "%.1f");
      }
      if (fieldIDs[field_id] == 1) {
        fieldNames[field_id] = "VOLTAGE";
        fieldValues[field_id] = valueRound(vescData.getVoltage(), "%.2f");
      }
      if (fieldIDs[field_id] == 2) {
        fieldNames[field_id] = "TRP DIST";
        fieldValues[field_id] = valueRound(
          vescData.correctedTripDistance,
          "%.1f"
        );
      }
      if (fieldIDs[field_id] == 3) {
        fieldNames[field_id] = "CURR";
        fieldValues[field_id] = valueRound(vescData.getCurrent(), "%.1f");
      }
      ...
      if (fieldIDs[field_id] == 5) {
        ...
        fieldValues[field_id] = valueRound(
          vescData.correctedTotalDistance,
          "%.1f"
        );
      }
      if (fieldIDs[field_id] == 6) {
        fieldNames[field_id] = "PWM";
        fieldValues[field_id] = valueRound(vescData.PWM, "%.1f");
      }
      if (fieldIDs[field_id] == 7) {
        fieldNames[field_id] = "BATT %";
        fieldValues[field_id] = valueRound(
          vescData.getBatteryPercentage(),
          "%.1f"
        );
      }
      if (fieldIDs[field_id] == 8) {
        fieldNames[field_id] = "BATT USG%"; //TODO
        fieldValues[field_id] = valueRound(vescData.batteryUsagePerc, "%.1f");
      }
      if (fieldIDs[field_id] == 9) {
        fieldNames[field_id] = "MAX TEMP";
        fieldValues[field_id] = valueRound(vescData.maxTemperature, "%.1f");
      }
      if (fieldIDs[field_id] == 10) {
        fieldNames[field_id] = "TOP SPD";
        fieldValues[field_id] = valueRound(vescData.topSpeed, "%.1f");
      }
      if (fieldIDs[field_id] == 11) {
        fieldNames[field_id] = "AVG SPD";
        fieldValues[field_id] = valueRound(vescData.avgMovingSpeed, "%.1f");
      }
      if (fieldIDs[field_id] == 12) {
        fieldNames[field_id] = "MIN VOLT";
        fieldValues[field_id] = valueRound(vescData.minVoltage, "%.2f");
      }
      if (fieldIDs[field_id] == 13) {
        fieldNames[field_id] = "MAX VOLT";
        fieldValues[field_id] = valueRound(vescData.maxVoltage, "%.2f");
      }
      if (fieldIDs[field_id] == 14) {
        fieldNames[field_id] = "MAX CURR";
        fieldValues[field_id] = valueRound(vescData.maxCurrent, "%.1f");
      }
      if (fieldIDs[field_id] == 15) {
        fieldNames[field_id] = "AVG CURR";
        fieldValues[field_id] = valueRound(vescData.avgCurrent, "%.1f");
      }
      if (fieldIDs[field_id] == 16) {
        fieldNames[field_id] = "MIN BATT %";
        fieldValues[field_id] = valueRound(
          vescData.lowestBatteryPercentage,
          "%.1f"
        );
      }
      if (fieldIDs[field_id] == 17) {
        fieldNames[field_id] = "MAX BATT %";
        fieldValues[field_id] = valueRound(vescData.maxBatteryPerc, "%.1f");
      }
      if (fieldIDs[field_id] == 18) {
        fieldNames[field_id] = "AVG PWR";
        fieldValues[field_id] = valueRound(vescData.avgPower, "%.1f");
      }
      if (fieldIDs[field_id] == 19) {
        fieldNames[field_id] = "MAX PWR";
        fieldValues[field_id] = valueRound(vescData.maxPower, "%.1f");
      }
      if (fieldIDs[field_id] == 20) {
        fieldNames[field_id] = "MAX PWM";
        fieldValues[field_id] = valueRound(vescData.maxPWM, "%.1f");
      }
      // GPS
      ...
      if (fieldIDs[field_id] == 25) {
        ...
        fieldValues[field_id] = valueRound(vescData.watchBatteryUsage, "%.1f");
      }
      //WATCH WEATHER
      ...
      if (fieldIDs[field_id] == 32) {
        ...
        if (current_weather != null) {
          fieldValues[field_id] = valueRound(vescData.batteryUsage, "%.1f");
        } else {
          fieldValues[field_id] = "--";
        }
      }
  ...
  function onUpdate(dc) {
    ...
    if (fieldNB == 6) {
      ...
      if (vescData.alternativeFont == true && vescData.limitedMemory == false) {
